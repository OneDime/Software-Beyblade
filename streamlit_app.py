...
def load_cloud_data():
    try:
        # Nota: read() usa i segreti configurati per l'accesso
...
inventario_corrente = st.session_state.users[user_selected]["inv"]
decks_correnti = st.session_state.users[user_selected]["decks"]
    ...
    for i, (_, row) in enumerate(filtered.iterrows()):
        with st.container(border=True):
            ...
            if st.button("Aggiungi tutto", key=f"all_{i}"):
                ...
                save_cloud_data() # TRIGGER SALVATAGGIO
        ...
        with st.expander(f"{deck['name'].upper()}", expanded=True):
            for s_idx in range(3):
                ...
                with st.expander(titolo_slot.upper(), expanded=st.session_state.exp_state.get(exp_key, False)):
                    ...
                    if deck["slots"][s_idx] != curr:
                        ...
                        save_cloud_data() # TRIGGER SALVATAGGIO (Cambio parti)
                        st.rerun()
            ...
            if c2.button("🗑️ Elimina", key=f"del_{user_selected}_{d_idx}", type="primary"):
                if len(decks_correnti) > 1:
                    ...
                    save_cloud_data() # TRIGGER SALVATAGGIO (Elimina Deck)
                    st.rerun()
